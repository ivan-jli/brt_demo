...
use esp_hal::{
    delay::Delay, gpio::{Event, Input, InputConfig, Io, Pull}, handler, i2c::master::*, main, rtc_cntl::{
        reset_reason, sleep::{GpioWakeupSource, TimerWakeupSource, WakeSource, WakeupLevel}, wakeup_cause, Rtc, Rwdt, RwdtStage, SocResetReason
...
fn main() -> ! {
    ...
    // i2c
    let mut i2c = I2c::new(peripherals.I2C0, Config::default()).unwrap().into_async()
    ...
    // Give the pinned future to Cassette for execution
    ...
    // let rtcio = GpioWakeupSource::default();  //:: ???????
    accel_int_in.wakeup_enable(true, esp_hal::gpio::WakeEvent::HighLevel).unwrap();
    // rtcio.apply(&rtc, triggers, sleep_config); //todo?
...
async fn accel_init<'a, Dm: DriverMode>(i2c: I2c<'a, Dm>) where esp_hal::i2c::master::I2c<'a, Dm>: embedded_hal_async::i2c::I2c {
